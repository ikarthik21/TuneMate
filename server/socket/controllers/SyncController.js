...
class SyncController {
    ...
    async sendConnectionRequest(targetUserId, username, senderId) {
        ...
        targetWs.send(JSON.stringify({ type: MESSAGE_TYPES.CONNECTION_REQUEST, payload: { username, senderId } }));
    }
    ...
    async acceptConnectionRequest(payload) {
        ...
        await redisClient.hset('activeConnections', sentBy.userId, acceptedBy.userId);
        await redisClient.hset('activeConnections', acceptedBy.userId, sentBy.userId);

        const targetWs = await this.getValidWebSocket(sentBy.userId);
        if (!targetWs) return;

        // Send the connection accepted message
        targetWs.send(JSON.stringify({ type: MESSAGE_TYPES.CONNECTION_ACCEPTED, payload: { acceptedBy: acceptedBy.username } }));
    }
    ...
    async declineConnectionRequest(payload) {
        ...
        targetWs.send(JSON.stringify({ type: MESSAGE_TYPES.CONNECTION_DECLINED, payload: { declinedBy: declinedBy.username } }));
    }

    async syncAction(payload) {
        console.log(payload);

        const { senderId, action, songId } = payload;
        ...
        if (targetWs) {
            targetWs.send(JSON.stringify({ type: action, payload: { songId } }));
        }
    }
}
