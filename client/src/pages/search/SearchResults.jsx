...
const SearchResults = () => {
  ...
  const renderTopResult = () => {
    ...
    return (
      <div className="flex m-2 flex-col">
        <h1 className="text-3xl jaro-head mt-2">Top Result</h1>
        {renderItem(
          topResult.type,
          topResult.id,
          <div className="flex items-center w-96 hover:bg-[#303033] p-4 rounded-xl cursor-pointer">
            ...
            <div className="flex flex-col ml-5">
              <h1 className="text-2xl mukta-medium">
                {truncateString(decodeHtmlEntities(topResult.title), 20)}
              </h1>
              {topResult.type === "song" && (
                <p className="text-xs montserrat-font mt-1">
                  {truncateString(decodeHtmlEntities(topResult.singers))}
                </p>
              )}
            </div>
          </div>
  ...
  const renderSongs = () =>
    data?.songs?.results.length > 0 && (
      <div className="flex justify-center p-2 flex-col">
        <h1 className="text-3xl jaro-head mt-2">Songs</h1>
            ...
            >
              ...
              <div className="flex flex-col ml-2">
                ...
                <p className="text-xs text-[#6a6a6a] nunito-sans-bold">
                  {truncateString(decodeHtmlEntities(song.singers))}
                </p>
              </div>
            </div>
          </div>
  ...
  const renderAlbums = () =>
    data?.albums?.results.length > 0 && (
      <div className="flex flex-col">
        <h1 className="text-3xl jaro-head ml-2">Albums</h1>
            ...
            >
              ...
              <div className="flex flex-col mt-1">
                <h3 className="mt-1 mb-1 nunito-sans-bold">
                  {truncateString(decodeHtmlEntities(album.title), 15)}
                </h3>
                <p className="text-xs text-[#6a6a6a] nunito-sans-bold">
  ...
  const renderArtists = () =>
    data?.artists?.results.length > 0 && (
      <div className="flex flex-col">
        <h1 className="text-3xl jaro-head ml-2">Artists</h1>
        <div className="flex items-center">
  ...
  const renderPlaylists = () =>
    data?.playlists?.results.length > 0 && (
      <div className="flex flex-col">
        <h1 className="text-3xl jaro-head ml-2">Playlists</h1>
        <div className="flex items-center">
