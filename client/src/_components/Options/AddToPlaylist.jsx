...
const AddToPlaylist = ({ clickEvent }) => {
    ...
    return (
        <div ref={addMenuRef} className={`rounded-lg w-64 z-50 bg-[#18181b] border-[#2D2E35] border p-4 absolute left-1 ${position}`}>
            <div className="flex flex-col">
                ...
                {!showCreatePlaylist && (
                    <div className="p-3 hover:bg-[#222328] hover:rounded border-b border-[#222328] flex items-center cursor-pointer" onClick={handleToggleCreatePlaylist}>
                ...
                <div className="flex flex-col mt-2 h-36 overflow-y-scroll custom-scrollbar">
                    <div className="flex items-center p-3 rounded hover:bg-[#222328] cursor-pointer justify-between">
                        ...
                        {Favorites.includes(songId) ? (
                            <MdFavorite size={22} cursor={"pointer"} color={"#59c2ef"} onClick={() => handleFavorite(songId)} />
                        ) : (
                            <IoMdAddCircle size={22} cursor={"pointer"} color={"#59c2ef"} onClick={() => handleFavorite(songId)} />
                        )}
                    </div>
