...
const Player = () => {
    const { song, isPlaying, getFavorites, loadPlayerState, playNext, playPrevious, AudioRef, handleAudioPlay, onLoop, handleSongLoop, isShuffling, handleShuffle, playSong } = usePlayerStore();
    const { connectWebSocket, closeWebSocket, socket, setConnectionStatus } = useWebSocketStore();
                ...
                try {
                    ...
                    switch (data.type) {
                        ...
                        case "CONNECTION_DECLINED":
                            Toast({ type: "error", message: `${data.payload.declinedBy} declined` });
                            break;
                        case "CONNECTION_ACCEPTED":
                            setConnectionStatus(true);
                            Toast({ type: "success", message: `${data.payload.acceptedBy} accepted` });
                            break;
                        case "PLAY_SONG":
    ...
    }, [playSong, setConnectionStatus, showNotifier, socket]);
        ...
        <div className="fixed bottom-0 bg-[#18181b] border-t border-[#2D2E35] left-0 w-full pr-4 pl-4 rounded text-amber-50 z-30">
            <div className="flex justify-between items-center">
                ...
                <div className="flex flex-col justify-center items-center flex-1">
                    <audio src={song?.downloadUrl[4]?.url} autoPlay ref={memoizedAudioRef}></audio>
                    <div className="mt-4 flex items-center justify-center">
                        <FaShuffle size={18} className="mr-8 cursor-pointer" onClick={handleShuffle} color={isShuffling ? "#59c2ef" : ""} />
                        <IoPlaySkipBack size={20} className="mr-8 cursor-pointer" onClick={playPrevious} />
                        <div className="bg-white mr-8 p-2 rounded-full flex items-center justify-center cursor-pointer" onClick={handleAudioPlay}>
                            {isPlaying ? <FaPause size={16} color="black" /> : <FaPlay size={15} color="black" className="relative left-[2px]" />}
                        </div>
                        <IoPlaySkipForward size={20} className="cursor-pointer" onClick={playNext} />
                        <MdOutlineLoop size={20} className="cursor-pointer ml-8" color={onLoop ? "#59c2ef" : ""} onClick={handleSongLoop} />
                    </div>
                    <div className="m-2">
